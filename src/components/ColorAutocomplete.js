...
const ColorAutocomplete = ({ query, onSelect }) => {
  const x11Colors = [
  ...
  const filteredColors = x11Colors.filter(color =>
  ...
  return (
    <div className="color-autocomplete-dropdown">
      {filteredColors.map((color, index) => (
        <div key={index} className="dropdown-item" onMouseDown={() => onSelect(color)}>
